...
void InstanceData::HandleGameObject(uint64 GUID, bool open, GameObject* go)
{
    ...
    if (go)
        ...
        sLog.outDebug("OSCR: InstanceData: HandleGameObject failed");
}
...
void InstanceData::DoUseDoorOrButton(uint64 uiGuid, uint32 uiWithRestoreTime, bool bUseAlternativeState)
{
    ...
    if (pGo)
    {
        if (pGo->GetGoType() == GAMEOBJECT_TYPE_DOOR || pGo->GetGoType() == GAMEOBJECT_TYPE_BUTTON)
        {
            ...
            sLog.outError("OSCR: Script call DoUseDoorOrButton, but gameobject entry %u is type %u.", pGo->GetEntry(), pGo->GetGoType());
    }
}
