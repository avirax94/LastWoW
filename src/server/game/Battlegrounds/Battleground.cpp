...
void Battleground::EndBattleground(uint32 winner)
{
    ...
    if (isArena() && isRated())
    {
        ...
        if (winner_arena_team && loser_arena_team)
        {
            ...
            DEBUG_LOG("Arena match Type: %u for Team1Id: %u - Team2Id: %u ended. WinnerTeamId: %u. Winner rating: %u, Loser rating: %u. RatingChange: %i.", m_ArenaType, m_ArenaTeamIds[BG_TEAM_ALLIANCE], m_ArenaTeamIds[BG_TEAM_HORDE], winner_arena_team->GetId(), winner_rating, loser_rating, winner_change);
            if (sWorld.getConfig(CONFIG_ARENA_LOG_EXTENDED_INFO))
                for (Battleground::BattlegroundScoreMap::const_iterator itr = GetPlayerScoresBegin(); itr != GetPlayerScoresEnd(); ++itr)
                    if (Player* player = sObjectMgr.GetPlayer(itr->first))
                        DEBUG_LOG("Statistics for %s (GUID: " UI64FMTD ", Team: %d, IP: %s): %u damage, %u healing, %u killing blows", player->GetName(), itr->first, player->GetArenaTeamId(m_ArenaType == 5 ? 2 : m_ArenaType == 3), player->GetSession()->GetRemoteAddress().c_str(), itr->second->DamageDone, itr->second->HealingDone, itr->second->KillingBlows);
        }
        else
        {
...
void Battleground::StartBattleground()
{
    ...
    SetLastResurrectTime(0);
    if (m_IsRated)
        DEBUG_LOG("Arena match type: %u for Team1Id: %u - Team2Id: %u started.", m_ArenaType, m_ArenaTeamIds[BG_TEAM_ALLIANCE], m_ArenaTeamIds[BG_TEAM_HORDE]);
