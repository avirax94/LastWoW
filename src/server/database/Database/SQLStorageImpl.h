...
void SQLStorageLoaderBase<DerivedLoader, StorageClass>::convert_from_str(uint32 /*field_pos*/, char* /*src*/, D& dst)
{
    dst = 0;
}
...
template<class DerivedLoader, class StorageClass>
void SQLStorageLoaderBase<DerivedLoader, StorageClass>::storeValue(char * value, StorageClass& store, char* p, uint32 x, uint32& offset)
{
    DerivedLoader* subclass = (static_cast<DerivedLoader*>(this));
    switch (store.GetDstFormat(x))
    {
    ...
    do
    {
        ...
        for (uint32 x = 0, y = 0; x < store.GetDstFieldCount();)
        {
            switch (store.GetDstFormat(x))
            {
                ...
                case FT_NA_POINTER: storeValue((char *)nullptr, store, record, x, offset); ++x; continue;
                default:
                    break;
            }
            ...
            switch (store.GetSrcFormat(y))
            {
                ...
                case FT_STRING: storeValue((char *)fields[y].GetString(), store, record, x, offset); ++x; break;
...
void SQLStorageLoaderBase<DerivedLoader, StorageClass>::LoadProgressive(StorageClass& store, uint32 wow_patch, std::string column_name /* = "patch" */, bool error_at_empty /*= true*/)
{
    ...
    do
    {
        ...
        for (uint32 x = 0, y = 0; x < store.GetDstFieldCount();)
        {
            ...
            case FT_STRING: storeValue((char *)fields[y + patchoffset].GetString(), store, record, x, offset); ++x; break;
