...
class SqlResultCallbackCaller : public ACE_Based::Runnable
{
    public:
        ...
        virtual void run()
        {
            ...
            mysql_thread_init();
            #endif
            Oregon::IQueryCallback* s = nullptr;
            while (queue.next(s))
            {
                s->Execute();
                delete s;
...
void SqlResultQueue::Update(uint32 timeout)
{
    ...
    Oregon::IQueryCallback* callback = nullptr;
    int n = 0;
    while (next(callback))
    {
    ...
    while (_threadUnsafeWaitingQueries.next(s))
    {
        ...
        delete s;
        --numUnsafeQueries;
        if (timeout && GetMSTimeDiffToNow(begin) > timeout)
            break;
    }
...
bool SqlQueryHolder::Execute(Oregon::IQueryCallback* callback, Database* database, SqlResultQueue* queue)
{
